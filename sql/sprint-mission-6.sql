...
SELECT * FROM orders
WHERE date = ('2025-03-12') AND time >= ('00:00:00') AND time < ('11:59:59')
ORDER BY date DESC, time DESC;
...
SELECT pizza_id, COUNT(order_id) AS count_order
FROM order_details
GROUP BY pizza_id;
...
SELECT pizza_id, COUNT(quantity) AS count_quantity
FROM order_details
GROUP BY pizza_id
...
SELECT o.pizza_id AS pizza_name ,SUM(o.quantity * p.price) AS total_revenue
...
FROM pizzas p
JOIN order_details o ON p.id = o.pizza_id
GROUP BY p.id
...
*/
SELECT o.date, COUNT(od.order_id) AS total_orders, SUM(p.price)
FROM orders o
...
/*
    3. `order`의 `id`가 78에 해당하는 주문 내역들을 조회합니다. 주문 내역에서 각각 주문한 피자의 이름을 `pizza_name`, 피자의 크기를 `pizza_size`, 
    피자 가격을 `pizza_price`, 수량을 `quantity`, 각 주문 내역의 총 금액을 `total_amount` 라는 이름으로 조회해 주세요.
...
*/
SELECT pt.name, p.size, p.price, od.quantity, (p.price * od.quantity) AS total_amount
FROM pizza_types pt
